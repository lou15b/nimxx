...
proc origin*(phs: LayoutVars): array[2, Expression] =
  [newExpression(phs.x), newExpression(phs.y)]
proc center*(phs: LayoutVars): array[2, Expression] =
  [phs.centerX, phs.centerY]
proc size*(phs: LayoutVars): array[2, Expression] =
  [newExpression(phs.width), newExpression(phs.height)]

proc topLeading*(phs: LayoutVars): array[2, Expression] =
  [phs.leading, newExpression(phs.y)]
proc bottomTrailing*(phs: LayoutVars): array[2, Expression] =
  [phs.trailing, phs.bottom]

proc frame*(phs: LayoutVars): array[4, Expression] =
  [newExpression(phs.x), newExpression(phs.y), newExpression(phs.width),
    newExpression(phs.height)]
...
proc inset*(fr: array[4, Expression], by: float32): array[4, Expression] {.inline.} =
  inset(fr, by, by)

# ***********************
# What is the role of the LayoutVars objects represented by
# prevPHS, nextPHS, superPHS, selfPHS???
# They are global objects - in the original code they were threadvars, but their
# initialization was at top level, so only the ones in the main thread were
# valid.
# As far as I can tell they are never mutated after initialization, so locks
# aren't needed even in a multi-threaded situation. The following code is
# intended to allow their use without complaints about GC safety.
...
proc autoresizingFrame*(
    leading, width, trailing, top, height, bottom: float32): array[4, Expression] =
