...
method init(v: CollectionsSampleView, r: Rect) =
  ...
  setTimeout 0.2, proc() =
    ...
    let popupDirectionRule = newPopupButton(v, newPoint(470, 20), newSize(100, 20),
      ["LeftToRight", "TopDown"])
    ...
    let popupLayoutWidth = newPopupButton(v, newPoint(470, 60), newSize(100, 20),
      ["Auto", "1", "2", "3", "4"])
    ...
    let popupItemSize = newPopupButton(v, newPoint(470, 100), newSize(100, 20),
      ["50", "100", "150"])
    popupItemSize.onAction do():
      collectionView.itemSize = newSize((50 + 50 * popupItemSize.selectedIndex()).Coord,
        (50 + 50 * popupItemSize.selectedIndex()).Coord)
      collectionView.updateLayout()
